<?php
/**
 * Created by JetBrains PhpStorm.
 * User: bluemooninc
 * Date: 2012/12/15
 * Time: 9:20
 * To change this template use File | Settings | File Templates.
 */
define('_AD_BMCART_PARENT_ID', '親カテゴリー');
define('_AD_BMCART_CATEGORY_EDIT', 'カテゴリー設定');
define('_AD_BMCART_CATEGORY_LIST', 'カテゴリーリスト');
define('_AD_BMCART_ITEM_LIST', '商品一覧表');
define('_AD_BMCART_TIPS_ITEM_EDIT', '個口配送料は、オーダーリスト中、最高値のみ摘要されます。');
define('_AD_BMCART_ITEM_NEW', '商品の追加');
define('_AD_BMCART_ITEM_EDIT', '商品の編集');
define('_AD_BMCART_ITEM_DELETE', '商品の削除');
define('_AD_BMCART_ITEM_DELETE_ADVICE', '商品を削除します。');
define('_AD_BMCART_ITEM', '商品');
define('_AD_BMCART_ITEM_ID', '商品ID');
define('_AD_BMCART_ITEM_CATEGORY', 'カテゴリー');
define('_AD_BMCART_ITEM_NAME', '商品名');
define('_AD_BMCART_ITEM_DESC', '商品概要');

define('_AD_BMCART_IMAGE_LIST', '商品画像一覧表');
define('_AD_BMCART_TIPS_IMAGE_EDIT', '商品の画像を登録します。１商品に何枚でも画像を設定出来ます。');
define('_AD_BMCART_IMAGE_EDIT', '画像の設定');
define('_AD_BMCART_IMAGE_NEW', '画像の追加');
define('_AD_BMCART_IMAGE_ID', '画像ID');
define('_AD_BMCART_IMAGE_FILENAME', '画像ファイル名');
define('_MD_BMCART_ERROR_IMAGE_EXTENSION', '画像ファイルの拡張子が違います。');
define('_MD_BMCART_ERROR_IMAGE_MAXFILESIZE', '画像ファイルのサイズが大きすぎます。');
define('_AD_BMCART_IMAGE_WEIGHT','表示ウエイト');

define('_AD_BMCART_CATEGORY', 'カテゴリ設定');
define('_AD_BMCART_CATEGORY_NEW', 'カテゴリの追加');
define('_AD_BMCART_CATEGORY_ID', 'カテゴリID');
define('_AD_BMCART_CATEGORY_NAME', '名称');
define('_AD_BMCART_CATEGORY_PARENT', '親カテゴリ');
define('_AD_BMCART_ITEM_STATUS', '状態');
define('_AD_BMCART_ITEM_TAG', 'タグ');
define('_AD_BMCART_ITEM_PERMISSION', 'アクセス制限');
define('_AD_BMCART_ITEM_STOCK', '在庫数量');
define('_AD_BMCART_ITEM_PRICE', '販売価格(税込)');
define('_AD_BMCART_ITEM_SHIPPING_FEE', '個口配送料');
define('_AD_BMCART_CONTROL', '操作');
define('_AD_BMCART_UPDATE', '更新日付');
define('_AD_BMCART_ORDER_LIST', '注文一覧表');
define('_AD_BMCART_TIPS_ORDER_EDIT', 'クレジット払いは商品発送時に決済課金処理を行います。');
define('_AD_BMCART_ORDERID', 'オーダーID');
define('_AD_BMCART_UID', 'UID');
define('_AD_BMCART_ADDRESS', '市区町村番地');
define('_AD_BMCART_PAYMENT_TYPE', 'お支払い方法');
define('_AD_BMCART_PAYBY_WIRE', '銀行振込');
define('_AD_BMCART_PAYBY_CREDIT', 'クレジットカード');
define('_AD_BMCART_PAYMENT_DESC', '料金明細');
define('_AD_BMCART_PAYMENT_SUB_TOTAL', '商品合計');
define('_AD_BMCART_PAYMENT_TAX', '消費税');
define('_AD_BMCART_PAYMENT_SHIPPING_FEE', '配送料');
define('_AD_BMCART_PAYMENT_AMOUNT', '請求金額');
define('_AD_BMCART_PAID_DATE','課金日時');
define('_AD_BMCART_ORDER_DATE', '注文日時');
define('_AD_BMCART_SHIPPING_DATE', '発送日');
define('_AD_BMCART_SHIPPING_CARRIER', '配送業者');
define('_AD_BMCART_SHIPPING_NUMBER', '伝票番号');
define('_AD_BMCART_ORDER_EDIT', '注文情報の編集');
define('_AD_BMCART_LAST_NAME', '姓');
define('_AD_BMCART_FIRST_NAME', '名');
define('_AD_BMCART_ADDRESS2', 'ビル・マンション等');
define('_AD_BMCART_ZIP_CODE', '郵便番号');
define('_AD_BMCART_STATE', '都道府県');
define('_AD_BMCART_SUB_TOTAL', '商品合計');
define('_AD_BMCART_TAX', '内消費税');
define('_AD_BMCART_SHIPPING_FEE', '配送料');
define('_AD_BMCART_AMOUNT', '合計金額');
define('_AD_BMCART_EXEC_TRANSIT', 'クレジット決済の実行');
define('_AD_BMCART_TIPS_TRANSIT', 'クレジット決済代行サービスのAPIより決済を実行します。');
define('_AD_BMCART_EXCEC_TRANSIT_DO', '決済実行');
define('_AD_BMCART_CARD_ORDER_ID', 'クレジット決済用オーダーID');
define('_AD_BMCART_SHIPPING_MAIL', '商品発送のお知らせ');
define('_AD_BMCART_TIPS_SHIPPING', '商品の発送をお客様にメール送信します。');
define('_AD_BMCART_USER_NAME', '発注ユーザー情報');
define('_AD_BMCART_SHIPPING_TO_NAME', '受取人氏名');
define('_AD_BMCART_SHIPPING_TO_ADDRESS', '受取人住所');
define('_AD_BMCART_SHIPPING_MAIL_DO', 'メール送信実行');
