...
class bmcart_ActionFrame
{
	...
	function bmcart_ActionFrame($admin)
	{
		$this->mAdminFlag = $admin;
		$this->mCreateAction =new XCube_Delegate();
		$this->mCreateAction->register('bmcart_ActionFrame.CreateAction');
	...
	function _createAction(&$actionFrame)
	{
		...
		//
		$className = "bmcart_" . ucfirst($actionFrame->mActionName) . "Action";
		$fileName = ucfirst($actionFrame->mActionName) . "Action";
		if ($actionFrame->mAdminFlag) {
			$fileName = XOOPS_MODULE_PATH . "/bmcart/admin/actions/${fileName}.class.php";
		}
		else {
			$fileName = XOOPS_MODULE_PATH . "/bmcart/actions/${fileName}.class.php";
		}
	...
	function execute(&$controller)
	{
		...
		if (!(is_object($this->mAction) && is_a($this->mAction, 'bmcart_Action'))) {
			die("action");	//< TODO
		}
